...
export interface DropdownGapsConfig {
...
interface DropdownGapsBlockProps {
  config: DropdownGapsConfig
}
...
function parseItems(items: string[]) {
  ...
  for (const item of items) {
    ...
    for (const match of item.matchAll(regex)) {
      ...
      blanks.push({ correct, options: rawOptions })
...
export default function DropdownGapsBlock({ config }: DropdownGapsBlockProps) {
  ...
  const score = useMemo(() => {
    ...
    return blanks.reduce((sum, blank, index) => {
      const isCorrect = answers[index]?.trim().toLowerCase() === blank.correct.toLowerCase()
        ...
        {parsedItems.map((item, itemIndex) => (
          <div key={`item-${itemIndex}`} className={config.mode === 'paragraph' ? 'leading-8' : ''}>
            ...
            {item.parts.map((part, partIndex) => {
              ...
              const selected = answers[part.blankIndex] ?? ''
              const isCorrect = checked && selected.trim().toLowerCase() === blank.correct.toLowerCase()
              ...
              return (
                <select
                  ...
                  aria-label={`Dropdown blank ${part.blankIndex + 1}`}
                >
